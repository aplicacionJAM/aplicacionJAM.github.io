// ==================== FUNCIÓN PARA TINTAR BARRA DE NAVEGACIÓN INFERIOR (ANDROID) ====================
    ...
    async function saveItem(store, item) {
        ...
        if (DATA_STORES.includes(store)) {
            ...
            try { await saveToIDB(store, D[store]); } catch(e) { console.warn('IDB save error, falling back to localStorage', e); }
            encolarSync(store, 'save', item);
        }
        saveToStorage(key, D[store] || []);
    }
    async function deleteItem(store, id) {
        const key = STORAGE_KEYS[store];
        if (DATA_STORES.includes(store)) {
            D[store] = (D[store] || []).filter(x => x.id !== id);
            try { await saveToIDB(store, D[store]); } catch(e) { console.warn('IDB delete error, falling back to localStorage', e); }
    ...
    async function getAll(store) {
        if (DATA_STORES.includes(store)) {
            try {
                const idb = await loadFromIDB(store);
                if(idb && idb.length > 0) return idb;
            } catch(e) {}
            const ls = loadFromStorage(STORAGE_KEYS[store], []);
            if(ls.length > 0) try { await saveToIDB(store, ls); } catch(e) {}
            return ls;
        }
        ...
        reader.onload = async function(e){
            try {
                ...
                for(let s of stores){
                    if(data[s]) {
                        D[s] = data[s];
                        if (DATA_STORES.includes(s)) { try { await saveToIDB(s, data[s]); } catch(e) {} }
                        localStorage.setItem(STORAGE_KEYS[s], JSON.stringify(data[s]));
                    }
                }
