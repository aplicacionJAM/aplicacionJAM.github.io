// ==================== UTILIDADES ====================
    ...
    function setNavigationBarColor(color) {
        let isDark = color === '#000000';
        let metaTheme = document.querySelector('meta[name="theme-color"]');
        if(metaTheme) metaTheme.setAttribute('content', color);
        let csMeta = document.getElementById('colorSchemeMeta');
        if(csMeta) csMeta.setAttribute('content', isDark ? 'dark' : 'light');
        document.documentElement.style.setProperty('color-scheme', isDark ? 'dark' : 'light');
    }
    
    // ==================== STORAGE KEYS ====================
    const DATA_STORES = ['productos', 'clientes', 'proveedores', 'gastos', 'empleados', 'ventas'];
    const STORAGE_KEYS = {
    ...
    let D = {
        ...
        config: { 
            ...
            ivaActivo:true, ivaPorcentaje:16, usarMargen:false, backgroundMode:'light', autoOscuro:false, prevenirCierre:true,
    ...
    function getNavBarColorFromMode() {
        return D.config.backgroundMode === 'dark' ? '#000000' : '#ffffff';
    }
    
    function applyTheme(){
        let isDark = D.config.backgroundMode === 'dark';
